...
var Cufon = (function() {
	...
	function f() {
		...
		this.get = function(H, I) {
			...
			K.sort(function(M, L) {
				return (E ? (M >= I && L >= I) ? M < L : M > L : (M <= I && L <= I) ?
					M > L : M < L) ? -1 : 1
			});
			return G[K[0]]
		}
	...
	function o(E, M, C, N, F, D) {
		...
		for (var J = 0, G = I.length; J < G; ++J) {
			H = z[N.engine](E, B ? n.textAlign(I[J], C, J, G) : I[J], C, N, F, D, J <
				G - 1);
			if (H) {
				K.appendChild(H)
			}
	...
	var z = {},
		...
		w = {
			...
			selector: (window.Sizzle || (window.jQuery && function(B) {
				...
				window.YAHOO && YAHOO.util && YAHOO.util.Selector && YAHOO.util.Selector
				.query) || (window.$$ && function(B) {
				return $$(B)
			}) || (window.$ && function(B) {
				return $(B)
...
Cufon.registerEngine("canvas", (function() {
	...
	var e = !a && (document.compatMode == "BackCompat" ||
	...
	f.appendChild(document.createTextNode((
		...
		"}@media print{cufon{padding:0;}cufon canvas{display:none;}}").replace(
		/;/g, "!important;")));
		...
		generate: for (var j = 0; k = o.exec(p); ++j) {
			...
			switch (k[1]) {
				case "v":
					g[j] = {
						m: "bezierCurveTo",
						a: [n + ~~l[0], m + ~~l[1], n + ~~l[2], m + ~~l[3], n += ~~l[4], m +=
							~~l[5]
						]
					};
					break;
	...
	return function(V, w, P, t, C, W) {
		...
		var E = V.spacing(Z, ~~m.convertFrom(parseFloat(P.get("letterSpacing")) ||
			0), ~~m.convertFrom(parseFloat(P.get("wordSpacing")) || 0));
		if (!E.length) {
			return null
		}
		var h = E.total;
...
Cufon.registerEngine("vml", (function() {
	...
	return function(ac, G, Y, C, K, ad, W) {
		...
		var I = ac.viewBox;
		var p = Y.computedFontSize || (Y.computedFontSize = new Cufon.CSS.Size(c(
			ad, Y.get("fontSize")) + "px", ac.baseSize));
		var y, q;
		if (n) {
			y = K;
			q = K.firstChild
